...
if __name__ == "__main__":
	...
	# Save merged results
	res_df.to_csv(
		os.path.join(paired_res_dir, 'merged_paired_results.csv'),
		index=False
	)
	...
	print(
		"Percentage of comparisons with nominal p < 0.01:",
		(res_df['p-val'] < 0.01).mean()
	)
	print(
	...
	print(
		"Percentage of comparisons with Bonferroni-corrected p < 0.01:",
		(res_df['p-val_bonf_corrected'] < 0.01).mean()
	)
	...
	for model in res_df['model'].unique():
		print(f"Comparisons for {model} with Bonferroni-corrected p < 0.01:")
		print(res_df[
				(res_df['model'] == model)
				& (res_df['p-val_bonf_corrected'] < 0.01)
			][
				['pheno', 'covars', 'p-val']
