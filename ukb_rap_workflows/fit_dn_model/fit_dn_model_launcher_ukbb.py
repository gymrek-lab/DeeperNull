...
* --val_samp_fname (str): Name of the validation sample file in samp_dir.
	Are not used in training, but will be predicted for. 
	Default: 'val_iids.txt'
* --test_samp_fname (str): Name of the test sample file in samp_dir.
	Are not used in training, but will be predicted for.
...
def parse_args():
	...
	parser.add_argument(
		'--val-samp-fname', type=str,
		default='val_iids.txt',
		help='Name of the validation sample file in samp_dir.'
	)
	parser.add_argument(
		'--test-samp-fname', type=str,
		default='test_iids.txt',
		help='Name of the test sample file in samp_dir.'
	)
...
def launch_fit(
	...
	val_samp_link = dxpy.dxlink(
		list(dxpy.find_data_objects(
			name=val_samp_file.split('/')[-1],
			folder='/'.join(val_samp_file.split('/')[:-1]),
			project=dxpy.PROJECT_CONTEXT_ID
		))[0]['id']
	)

	test_samp_link = dxpy.dxlink(
		list(dxpy.find_data_objects(
			name=test_samp_file.split('/')[-1],
			folder='/'.join(test_samp_file.split('/')[:-1]),
			project=dxpy.PROJECT_CONTEXT_ID
		))[0]['id']
	)
	...
	workflow_input = {
		...
		f"{prefix}val_samp_file": val_samp_link,
		f"{prefix}test_samp_file": test_samp_link,
		f"{prefix}pat": pat
	}
