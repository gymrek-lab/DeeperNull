...
if __name__ == "__main__":
	...
	for pheno in PHENO_ALIAS.keys():
		...
		rows = add_r2_row(
			pheno, 
			"Null: XGB, age, sex, & spatiotemporal", 
		...
		rows = add_r2_row(
			pheno,
			"BASIL PGS: XGB null & spatiotemporal covariates",
	...
	# Compute percent improvement of PRS models over null models

	improvement_df = scores_df.pivot(
		index=['pheno'],
		columns='desc',
		values='R^2'
	).reset_index()

	# First, imporvement of standard PRS then extended PRS over linear null, then over XGB null
	improvement_df['perc imp: std PRS vs lin null'] = (
		improvement_df['BASIL PGS: standard'] - improvement_df['Null: Linear, age & sex']
	) / improvement_df['Null: Linear, age & sex'] * 100
	improvement_df['perc imp: std PRS vs XGB null'] = (
		improvement_df['BASIL PGS: standard'] - improvement_df['Null: XGB, age, sex, & spatiotemporal']
	) / improvement_df['Null: XGB, age, sex, & spatiotemporal'] * 100
	improvement_df['perc imp: extended PRS vs lin null'] = (
		improvement_df['BASIL PGS: XGB null & spatiotemporal covariates'] - improvement_df['Null: Linear, age & sex']
	) / improvement_df['Null: Linear, age & sex'] * 100
	improvement_df['perc imp: extended PRS vs XGB null'] = (
		improvement_df['BASIL PGS: XGB null & spatiotemporal covariates'] - improvement_df['Null: XGB, age, sex, & spatiotemporal']
	) / improvement_df['Null: XGB, age, sex, & spatiotemporal'] * 100

	# Print median percent improvements
	print('Median percent improvement of standard PRS over linear null:')
	print(improvement_df['perc imp: std PRS vs lin null'].median())
	print('Median percent improvement of standard PRS over XGB null:')
	print(improvement_df['perc imp: std PRS vs XGB null'].median())
	print('Median percent improvement of extended PRS over linear null:')
	print(improvement_df['perc imp: extended PRS vs lin null'].median())
	print('Median percent improvement of extended PRS over XGB null:')
	print(improvement_df['perc imp: extended PRS vs XGB null'].median())

	print(improvement_df[[
		'pheno',
		'perc imp: std PRS vs lin null',
		'perc imp: std PRS vs XGB null',
		'perc imp: extended PRS vs lin null',
		'perc imp: extended PRS vs XGB null',
	]])
