...
def print_percent_improvement(
	...
	print('Median percent improvement over baseline:')
	print(
		improvement_df.groupby(group_col)['percent_change'].median().sort_values(
			ascending=False
		)
	)
	# print('Mean percent improvement over baseline:')
	# print(
	# 	improvement_df.groupby(group_col)['percent_change'].mean().sort_values(
	# 		ascending=False
	# 	)
	# )
...
if __name__ == '__main__':
	...
	merged_df = merged_df.sort_values(
		['pheno','covar_set', metric], ascending=[True, True, is_mae]
	...
	# Plot like above, but shape for model
	merged_df['model_class'] = 'Linear'
	merged_df.loc[
		merged_df['model_type'].str.contains('DeepNull'), 'model_class'
	] = 'Neural Network'
	merged_df.loc[
		merged_df['model_type'].str.contains('XGBoost'), 'model_class'
	] = 'XGBoost'

	# Take only the best per pheno-covar set to reduce clutter
	merged_df = merged_df.sort_values(
		['pheno','covar_set', metric], ascending=[True, True, is_mae]
	).drop_duplicates(['pheno','Covariate Set'])
	...
	ax = sns.scatterplot(
		...
		palette=base_palette,
		style='model_class',
		style_order=['Linear', 'Neural Network', 'XGBoost'],
		markers={'Linear': '^', 'Neural Network': 'v', 'XGBoost': 'd'},
		s=60,
		alpha=0.8,
		edgecolor='none',
		linewidth=0,
